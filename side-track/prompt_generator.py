...
class PromptGenerator:
        ...
        try:
            ...
            if datasource["type"] == "s3":
                prompt += (
                    ...
                    f"For year-only date filters, use {date_function} = value. For full date filters, use column = 'YYYY-MM-DD'. "
                    f"Use LOWER and LIKE for strings, SUM and AVG for numerics. "
    ...
    def generate_user_prompt(self, datasource: Dict, nlq: str, schemas: List[str], entities: Optional[Dict] = None, prediction: Optional[Dict] = None, user_role: str = "user") -> str:
        ...
        try:
            ...
            extracted_values = self.nlp_processor.process_query(nlq, schemas[0], datasource=datasource).get("extracted_values", {})
            ...
            prompt = (
                ...
                f"Extracted Values: {json.dumps(extracted_values, indent=2)}\n"
            ...
            if datasource["type"] == "s3":
                ...
                for table in prediction.get("tables", []):
                    ...
                    try:
                        ...
                        continue
                date_instruction = (
                    f"For year-only date filters (e.g., dates_type='year'), use {date_function} = value. "
                    f"For full date filters (e.g., dates_type='full_date'), use column = 'YYYY-MM-DD'. "
                )
                prompt += (
        ...
        try:
            ...
            for table in tables_data:
                ...
                for col in table.get("columns", []):
                    ...
                    # Add date columns for date-related queries
                    if "date" in col_name.lower() and any(token in ["year", "date"] for token in tokens):
                        columns.append(col["name"])
    ...
    def generate_sql(self, datasource: Dict, system_prompt: str, user_prompt: str, schemas: List[str], entities: Dict, tia_result: Dict, user_role: str = "user") -> str:
        ...
        try:
            ...
            if not sql:
                ...
                extracted_values = self.nlp_processor.process_query(user_prompt.split("\n")[0].replace("User Query: ", ""), schemas[0], datasource=datasource).get("extracted_values", {})
                if extracted_values.get("dates_type") == "year" and extracted_values.get("dates_year"):
                    # Fallback SQL for year-only queries, assuming date column is string
                    date_column = "order_date"  # Default date column based on metadata
                    year_value = extracted_values["dates_year"]
                    table_name = tia_result.get("tables", [f"{schemas[0]}.orders"])[0].split(".")[-1]
                    sql = f"SELECT * FROM {table_name} WHERE strftime(TRY_CAST(strptime({date_column}, '%Y-%m-%d') AS DATE), '%Y') = '{year_value}'"
                    self.logger.debug(f"Generated fallback SQL for year-only query: {sql}")
                else:
